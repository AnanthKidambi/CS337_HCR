...
# _style_wt_list = [1e3/5 for n in [64,128,256,512,512]]
...
class extractor:
    ...
    def __call__(self, img):
        ...
        # gram = {key: torch.matmul(val, val.t())/val.shape[1] for key, val in flat.items()}
        assert all(val1.shape[2] * val1.shape[3] == val2.shape[1] for val1, val2 in zip(style_out.values(), flat.values()))

        # gram = {}
        # for key, val in style_out.items():
        #     b,c,h,w = val.size()
        #     F = val.view(b, c, h*w)
...
def closure():
    ...
    for key, val in style_outputs.items():
        ...
        loss += style_weights[key] * nn.functional.mse_loss(style_outputs[key], actual_gram_matrices[key])
